...
/// Services conforming to this protocol are marked `@unchecked Sendable` and have mutable configuration properties.
/// Services should be configured on a single thread/context before being shared across concurrent contexts.
/// Mutating configuration properties (`name`, `systemPrompt`, `defaultModel`, etc.) from multiple threads
/// simultaneously is unsafe and can cause data races.
///
...
public protocol LLMServiceProtocol: Sendable {
    ...
    ///    -  onPartialResponse: A closure that handles partial responses during streaming. Must be @Sendable for Swift 6 concurrency safety.
    ///
