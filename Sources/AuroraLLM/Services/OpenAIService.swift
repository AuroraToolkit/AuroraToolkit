...
/// }
/// ```
public class OpenAIService: LLMServiceProtocol, @unchecked Sendable {
    ...
    /// StreamingDelegate handles URLSession streaming responses for OpenAI legacy chat API.
    ...
    /// ResponsesStreamingDelegate handles URLSession streaming responses for OpenAI Responses API.
    /// Marked as @unchecked Sendable because URLSession delegates are called serially from URLSession's internal queue,
    /// ensuring thread-safe access to mutable state.
    final class ResponsesStreamingDelegate: NSObject, URLSessionDataDelegate, @unchecked Sendable {
        private let vendor: String
        private let model: String
        private let onPartialResponse: @Sendable (String) -> Void
        private let continuation: CheckedContinuation<LLMResponseProtocol, Error>
